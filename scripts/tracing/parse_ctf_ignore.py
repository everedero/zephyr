...
def parse_args():
    ...
    parser.add_argument("-i", "--ignore",
            ...
            default=False,
            help="ignore parser errors (useful for trace buffers with" +
                "0-padding or invalid data)")
    args = parser.parse_args()
    return args

def strip_zero_padding(folder):
    with open(folder + "/mytrace.ctf", "rb") as myfile:
        content = myfile.read()
    for idx in range(len(content)): # Yeah, rstrip but with non-ascii
        if content[len(content)-1-idx] != 0: # Breaking at first non-null
            break
    content = content[0:len(content)-idx]

    with open(folder + "/mytrace.ctf", "wb") as myfile:
        myfile.write(content)

def wrapper(gen):
    """
    Add exception handling to iterator
    """
    while True:
        try:
            yield next(gen)
        except bt2._Error as e:
            logging.warning("Aborting parsing before EOF")
            return()
...
def main():
    ...
    if args.ignore:
        logging.info("Ignoring parsing error")
        msg_it = wrapper(msg_it)
